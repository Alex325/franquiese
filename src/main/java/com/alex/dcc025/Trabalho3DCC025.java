...
public class Trabalho3DCC025 {
    public static void main(String[] args) {
        ...
        dono.getGerente(0).getVendedor(0).cadastrarPedido("null", List.of(new ItemPedido(dono.getGerente(0).getFranquia().getEstoque().get(0), 1), new ItemPedido(dono.getGerente(0).getFranquia().getEstoque().get(0), 1), new ItemPedido(dono.getGerente(0).getFranquia().getEstoque().get(0), 1), new ItemPedido(dono.getGerente(0).getFranquia().getEstoque().get(0), 1)), FormaPagamento.CREDITO, ModalidadeEntrega.ENTREGA);
